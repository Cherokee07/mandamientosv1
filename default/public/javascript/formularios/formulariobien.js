$(document).ready(function() {
	...
	function llenarnombres(){ // funcion para llenar el select de nombres de propietarios
		...
		if($('#propietarios').val()=='1'){ // se llena de ofendidos si es 1
			$.post(url_base+"ofendidostemp/listaOfendidos",{'idexp':idexp})
		...
		if($('#propietarios').val()=='2'){ // se llena de imputados si es 2
			$.post(url_base+"indiciadostemp/listaImputados", {'idexp':idexp})
	        .done(function(result){
	        	if(JSON.parse(result).length>0){
		         $.each(JSON.parse(result), function(k, v) {
...
function eliminarB(event){
    ...
    $.post(url_base+"bienesaseguradostemp/eliminarbienTemp", {'idbien':id})
    .done(function(result){
      $('#row-'+id).remove();
    })
    .fail(function(err){
